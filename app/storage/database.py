...
class Database:
    ...
    def __init__(self, uri='', database='kaeleon', client=None):
        self.client=client or (MongoClient(uri, serverSelectionTimeoutMS=3000) if uri else None)
        self.db=self.client[database] if self.client else None
        self.memory=[]
        if self.db: self.ensure_indexes()
    ...
    def write(self, collection, document):
        ...
        if self.db: return self.db[collection].insert_one(document).inserted_id
    ...
    def find_one(self, collection, key):
        if self.db: return self.db[collection].find_one(key)
