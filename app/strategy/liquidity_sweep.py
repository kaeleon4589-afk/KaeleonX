    ...
    def evaluate(self,regime,candles,decision_id,symbol,timeframe,current_price=None,snapshot=None):
        ...
        for i in range(start,len(c)-1):
            ...
            if x.low<lo and x.close>=lo-a*.36:
                ...
                if valid:
                    rr=max((max(z.high for z in c[max(0,i-48):i])-p)/max(p-(x.low-a*.18),1e-12),0); score=self._score(depth,wick,rv,trv,tb,closepos,ext,rr,age); cand=('long',score,x.low-a*.18,lo,depth,wick,rv,trv,rr,age)
            ...
            if x.high>hi and x.close<=hi+a*.36:
                ...
                if valid:
                    rr=max((p-min(z.low for z in c[max(0,i-48):i]))/max((x.high+a*.18)-p,1e-12),0); score=self._score(depth,wick,rv,trv,tb,closepos,ext,rr,age); cand=('short',score,x.high+a*.18,hi,depth,wick,rv,trv,rr,age)
                    if best is None or score>best[1]:best=cand
        if not best:
        ...
        self.last_trace={'accepted':True,'reason':'setup_valid','score':score,'side':side,'sweep_level':level,'depth_atr':depth,'wick_ratio':wick,'sweep_rvol':srv,'trigger_rvol':trv,'rr':rr,'age':age,'sl_pct':sl_pct,'tp_pct':tp_pct,'entry':p,'stop':stop,'target':target}
